...
class _TransactionFormScreenState extends ConsumerState<TransactionFormScreen> {
  ...
  String get _displayAmount {
    ...
    final grouped =
        NumberFormat('#,##0', 'tr_TR').format(int.tryParse(intPart) ?? 0);
    final frac = parts.length == 2 ? ',${parts[1]}' : (_amount.endsWith(',') ? ',' : '');
  ...
  Future<void> _pickAccount({required bool related}) async {
    ...
    final id = await showPickerSheet<int>(
      context,
      title: related ? 'Hedef hesap' : 'Hesap',
      options: options,
  ...
  String? _validate() {
    if (_cents <= 0) return 'Tutar gir';
    if (_account == null) return 'Hesap seç';
    if (_kind == 'transfer') {
      if (_relatedAccount == null) return 'Hedef hesap seç';
      if (_relatedAccount!.id == _account!.id) {
        return 'Kaynak ve hedef hesap farklı olmalı';
      }
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      setState(() {
        _saving = false;
        _error = 'Kaydedilemedi, tekrar dene';
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text(_isEdit ? 'İşlemi Düzenle' : 'İşlem Ekle'),
      ),
