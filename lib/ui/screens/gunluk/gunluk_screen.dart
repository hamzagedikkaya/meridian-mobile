...
class GunlukScreen extends ConsumerStatefulWidget {
  const GunlukScreen({super.key});

  @override
  ConsumerState<GunlukScreen> createState() => _GunlukScreenState();
}

class _GunlukScreenState extends ConsumerState<GunlukScreen> {
  String _range = '30d';
  String? _moodFilter;
        ...
        SliverList.builder(
          ...
          itemBuilder: (context, index) {
            ...
            Widget item = Padding(
              ...
              child: JournalCard(
                entry: entry,
                onTap: () => context.push('/gunluk/${entry.id}'),
              ),
            );
            if (stagger && index < 6) {
              item = item
  ...
  Widget _skeleton(BuildContext context) {
    final placeholder = JournalEntry(
      id: 0,
      date: DateTime.now(),
      title: 'Örnek bir günlük başlığı',
      bodyPlain:
          'Bugün oldukça sakin geçti. Sabah yürüyüşe çıktım ve gün boyu '
          'enerjim yüksek kaldı. Akşam biraz kitap okudum.',
      mood: 'good',
      moodEmoji: '🙂',
      energyLevel: 3,
      tags: const ['etiket', 'gün'],
      hasGratitude: true,
    );
    return NokSkeleton(
      enabled: true,
  ...
  Widget _emptyView(BuildContext context) => EmptyState(
        icon: Icons.auto_stories_outlined,
        title: 'Bugün nasıldı?',
        subtitle: 'İlk kaydını yaz',
        actionLabel: 'Kayıt oluştur',
        onAction: () => openJournalEditor(context),
      );

  Widget _errorView(BuildContext context) => ListView(
        children: [
          SizedBox(
