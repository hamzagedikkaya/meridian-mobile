...
const _weatherOptions = <(String, IconData)>[
  ('Güneşli', Icons.wb_sunny_outlined),
  ('Parçalı bulutlu', Icons.wb_cloudy_outlined),
  ('Bulutlu', Icons.cloud_outlined),
  ('Yağmurlu', Icons.grain),
  ('Karlı', Icons.ac_unit),
];

IconData _weatherIcon(String? weather) {
  for (final (label, icon) in _weatherOptions) {
    if (label == weather) return icon;
  }
...
class _JournalEditorScreenState extends ConsumerState<JournalEditorScreen> {
  ...
  Future<void> _pickWeather() async {
    final picked = await showPickerSheet<String>(
      ...
      options: [
        for (final (label, icon) in _weatherOptions)
          PickerOption(value: label, label: label, icon: icon),
      ],
    );
  ...
  Future<void> _save() async {
    ...
    if (title.isEmpty && body.isEmpty) {
      showAppSnack(context, 'Bir başlık ya da birkaç satır yaz', isError: true);
      return;
    }
    ...
    try {
      ...
      if (!mounted) return;
      Haptics.success();
      showAppSnack(context, _isEditing ? 'Güncellendi ✓' : 'Kayıt eklendi ✓');
      ...
      // Any non-API failure must still clear the spinner, or "Kaydet" stays a
      // permanent loading state.
      if (!mounted) return;
      setState(() => _saving = false);
      showAppSnack(context, 'Kayıt kaydedilemedi', isError: true);
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text(_isEditing ? 'Düzenle' : 'Yeni kayıt'),
        actions: [
          _saving
              ? Padding(
      ...
      bottomNavigationBar: _AccessoryBar(
        ...
        weatherIcon: _weatherIcon(_weather),
        energy: _energy,
            ...
            children: [
              ...
              if (_tagsOn) ...[
                ...
                TextField(
                  ...
                  decoration: const InputDecoration(
                    hintText: 'virgülle ayır: yürüyüş, aile',
                  ),
                ),
              ],
              if (_gratitudeOn) ...[
                const SizedBox(height: 16),
                _overline(context, 'MİNNET'),
                const SizedBox(height: 8),
                TextField(
                  ...
                  textCapitalization: TextCapitalization.sentences,
                  decoration: const InputDecoration(
                    hintText: 'Bugün ne için minnettarsın?',
                  ),
                ),
              ],
            ],
        ...
        children: [
          Container(
            ...
            child: Row(
              ...
              children: [
                Icon(_weatherIcon(_weather), size: 14, color: c.inkMid),
        ...
        child: Padding(
          ...
          child: Row(
            children: [
              ...
              const Spacer(),
              Text('Enerji', style: text.labelSmall),
              const SizedBox(width: 8),
