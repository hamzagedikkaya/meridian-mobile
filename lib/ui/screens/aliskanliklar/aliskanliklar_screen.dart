...
class AliskanliklarScreen extends ConsumerStatefulWidget {
  const AliskanliklarScreen({super.key});

  @override
  ConsumerState<AliskanliklarScreen> createState() =>
      _AliskanliklarScreenState();
}

class _AliskanliklarScreenState extends ConsumerState<AliskanliklarScreen> {
  HabitsBundle? _local;
  ...
  Future<void> _toggle(Habit habit, int delta) async {
    ...
    try {
      ...
      showAppSnack(context, 'İşaretlenemedi, tekrar dene', isError: true);
    } finally {
      ...
      builder: (_) => SafeArea(
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              Text('Mükemmel gün geçmişi', style: text.titleLarge),
              const SizedBox(height: 16),
              PerfectDayDots(chain: bundle.perfectDay.chain, dot: 12),
              const SizedBox(height: 16),
              Text(
                'Mükemmel seri: 🔥 ${bundle.perfectDay.currentStreak} · '
                'Rekor: ${bundle.perfectDay.longestStreak}',
        ...
        slivers: [
          ...
          SliverFillRemaining(
            ...
            child: EmptyState(
              ...
              title: 'İlk alışkanlığını oluştur',
              subtitle: 'Küçük başla',
          ...
          Column(
            ...
            children: [
              Row(
                children: [
                  Expanded(
                    child: Text('$done / $total tamamlandı',
  ...
  Widget _perfectChainCard(BuildContext context, HabitsBundle bundle) {
    ...
    return NokturnCard(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('Mükemmel gün zinciri', style: text.titleMedium),
          const SizedBox(height: 12),
          PerfectDayDots(chain: pd.chain),
          const SizedBox(height: 12),
          Text(
            'Mükemmel seri: 🔥 ${pd.currentStreak} · Rekor: ${pd.longestStreak}',
            style: text.bodySmall!.copyWith(color: c.inkMid),
          ),
        ],
      ),
        ...
        children: [
          NokturnCard(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('3 / 7 tamamlandı', style: text.titleLarge),
                const SizedBox(height: 12),
                Container(
                  height: 8,
                  decoration: BoxDecoration(
